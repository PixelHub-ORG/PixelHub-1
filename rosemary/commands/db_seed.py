...
def get_module_seeders(module_path, specific_module=None):
    ...
    for root, dirs, files in os.walk(module_path):
        if "seeders.py" in files:
            ...
            if specific_module and specific_module != module_name.split(".")[
                    0]:
                continue
...
def db_seed(reset, yes, module):
    ...
    if reset:
        if yes or click.confirm(
            click.style(
                "This will reset the database, do you want "
                "to continue?",
                fg="red"),
            abort=True,
        ):
    ...
    if module:
        click.echo(
            click.style(
                f"Seeding data for the '{module}' module...",
                fg="green"))
    else:
