...
# falla
# def test_orcid_callback_token_error(test_client, monkeypatch):
...
#     response = test_client.get("/orcid/callback", follow_redirects=True)
    
#     assert response.status_code == 200
#     assert b"ORCID login failed" in response.data  # Comparar directamente con bytes


# def test_orcid_callback_no_orcid_id(test_client, monkeypatch):
#     def mock_authorize_access_token():
...
#     response = test_client.get("/orcid/callback", follow_redirects=True)
#     assert response.status_code == 200
#     assert b"Could not retrieve ORCID iD" in response.data


# def test_orcid_callback_user_creation_error(test_client, monkeypatch):
#     def mock_authorize_access_token():
#         return {"orcid": "0000-0001-2345-6789", "name": "Test User"}
    
#     def mock_find_or_create_by_orcid(orcid_id, full_name):
...
#     response = test_client.get("/orcid/callback", follow_redirects=True)
#     assert response.status_code == 200
#     assert b"Error creating user profile" in response.data
