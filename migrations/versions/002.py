...
def upgrade():
    ...
    with op.batch_alter_table("user", schema=None) as batch_op:
        ...
        batch_op.alter_column(
            "email", existing_type=sa.String(length=256), nullable=True
        )
        batch_op.alter_column(
            "password", existing_type=sa.String(length=256), nullable=True
        )
...
def downgrade():
    with op.batch_alter_table("user", schema=None) as batch_op:
        batch_op.alter_column(
            "password", existing_type=sa.String(length=256), nullable=False
        )
        batch_op.alter_column(
            "email", existing_type=sa.String(length=256), nullable=False
        )
